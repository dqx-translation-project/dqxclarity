...
class Pymem(object):
    ...
    def __init__(
        ...
        if process_name is not None:
            if isinstance(process_name, str):
                ...
                raise TypeError(f"process_name must be of type int or string not {type(process_name).__name__}")
    ...
    def inject_python_interpreter(self, initsigs=0):
        ...
        def find_existing_interpreter(_python_version):
            ...
            self.py_run_simple_string = module.lpBaseOfDll + (
        ...
        # Workaround: inject_dll is returning the wrong address to the python39.dll in the process.
        #             But, when we check for the interpreter after injection, it finds the correct
        #             address. We check again for the address here.

        python_lib_h = find_existing_interpreter(python_version)

        local_handle = pymem.ressources.kernel32.GetModuleHandleW(python_lib)
        py_initialize_ex = python_lib_h + (
            pymem.ressources.kernel32.GetProcAddress(local_handle, b"Py_InitializeEx") - local_handle
        )
        self.py_run_simple_string = python_lib_h + (
    ...
    def inject_python_shellcode(self, shellcode):
        ...
        pymem.ressources.kernel32.WriteProcessMemory(
            self.process_handle, shellcode_addr, shellcode, len(shellcode), ctypes.byref(written)
        )
        # check written
        self.start_thread(self.py_run_simple_string, shellcode_addr)
        # Return both the py_run_simple_string address and shellcode_addr
        dic = {}
        dic["py_run_simple_string_addr"] = self.py_run_simple_string
        dic["shellcode_addr"] = shellcode_addr
    ...
    def start_thread(self, address, params=None):
        ...
        thread_h = pymem.ressources.kernel32.CreateRemoteThread(
            self.process_handle, NULL_SECURITY_ATTRIBUTES, 0, address, params, 0, ctypes.byref(ctypes.c_ulong(0))
        )
    ...
    def pattern_scan_module(self, pattern, module, *, return_multiple=False):
        ...
        return pymem.pattern.pattern_scan_module(self.process_handle, module, pattern, return_multiple=return_multiple)
    ...
    def read_ctype(self, address, ctype, *, get_py_value=True, raw_bytes=False):
        ...
        if not self.process_handle:
            raise pymem.exception.ProcessError("You must open a process before calling this method")
        try:
            value = pymem.memory.read_ctype(
                self.process_handle, address, ctype, get_py_value=get_py_value, raw_bytes=raw_bytes
            )
        except pymem.exception.WinAPIError as e:
