...
internal static class LocaleEmulatorService
{
    ...
    // Optional postInject callback runs with the still-open process handle so callers can
    // inject additional dlls (e.g. PacketWarden) without re-implementing CreateProcess.
    internal static bool Launch(string applicationName, string? arguments, string workingDirectory,
                                Action<IntPtr>? postInject = null)
    {
    ...
    // Shared injection primitive — usable by anything that has a process handle
    // and a 32-bit dll path. `log` is optional; called with per-step diagnostics
    // so callers can surface failures.
    internal static bool InjectWithRetry(IntPtr hProcess, string dllPath, Action<string>? log = null)
    {
        for (int i = 0; i < 20; i++)
        {
            Thread.Sleep(50);
            ...
            {
                if (i == 0 || i == 19)
                    log?.Invoke($"attempt {i + 1}: kernel32 not yet enumerable in target (win32 err {Marshal.GetLastWin32Error()})");
                continue;
            }
            if (TryInject(hProcess, dllPath, (IntPtr)(long)loadLibW, out var err))
                return true;
            log?.Invoke($"attempt {i + 1}: {err}");
        }
        return false;
    }

    private static bool TryInject(IntPtr hProcess, string dllPath, IntPtr loadLibW, out string err)
    {
        err = "";
        ...
        {
            err = $"VirtualAllocEx failed (win32 err {Marshal.GetLastWin32Error()})";
            return false;
        }
        try
        {
            ...
            {
                err = $"WriteProcessMemory failed (win32 err {Marshal.GetLastWin32Error()})";
                return false;
            }
            var thread = CreateRemoteThread(hProcess, IntPtr.Zero, 0, loadLibW, remote, 0, out _);
            if (thread == IntPtr.Zero)
            {
                err = $"CreateRemoteThread failed (win32 err {Marshal.GetLastWin32Error()})";
                return false;
            }
            ...
            if (code == 0)
            {
                err = "LoadLibraryW in the target returned 0 — dll path was reachable but Windows refused to load it (architecture mismatch, missing dep, or AV).";
                return false;
            }
            return true;
        }
